---
title: "Fusta DEF - Defensa"
description: "Les fustes DEF són les més lentes del mercat, dissenyades per a jugadors defensius que prioritzen el control màxim, la col·locació precisa i l'ús de pics llargs o antitop."
pubDate: "2025-08-03"
updatedDate: "2025-09-02"
category: "fustes"
tags: ["Fustes"]
draft: false
---

[

![Butterfly Innershield Layer](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjnSxxeRHoHMpRd-VIvizf8CDteDH5W432tKZ7S8D-3-pbFLCPVyutYINSrUmLXoxCYjTmxSwIAq1uWeif1VgrAS8IjXaK4Ldu5boQMeuL1xBiD418xzcH5e7TzJgEV28pQy8XdryGNUHayY9j347b4J6JzUKD-sm37QqBn3WdwwghIP4l-95C_7cuIGe9G/w239-h400/fusta%20Butterfly%20Innershield%20Layer.jpg)

]()
|  |
| --- |
| Fusta Butterfly Innershield Layer |

**Què són les fustes DEF (defensa):**
Pales pensades per a jugadors defensius. Són lentes, lleugeres i ofereixen molt control. S’utilitzen per tallar, bloquejar i retornar amb precisió.

Ideal per a:
- Estil defensiu clàssic o tallador.
- Jugadors que fan servir pics llargs o antitop.
- Control màxim i pilotes molestes per al rival.

Avantatges:
- Moltíssim control.
- Molt bona absorció de velocitat.
- Permet col·locar la pilota amb precisió.

Inconvenients:
- Molt poca velocitat.
- Difícils d’utilitzar en atac o contraatac.
- Requereix una bona lectura del joc i paciència.

Exemples:
- Butterfly Innershield Layer ZLF
- Stiga Defensive Classic
- Donic Defplay Senso V3
- TSP Balsa 3.5